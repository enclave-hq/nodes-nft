...
### 临时调整规则
- **规则**: 因为调整活跃订单和我的挂单的UI，所做的临时调整（如临时测试数据），在UI调整完以后，**必须改回去**
- **适用范围**: 
  - `frontend/lib/hooks/useMarketplace.ts` 中的临时测试数据
  - 任何为UI调试而添加的临时代码
- **标记方式**: 所有临时调整必须使用 `// TODO: 临时测试数据 - UI调整完成后需要删除` 或类似的注释标记
- **清理时机**: UI调整完成后立即清理所有临时代码，恢复原始实现
- **当前临时调整位置**: 无（已清理完成）
