...
function NFTCard({ nftId }: { nftId: number }) {
  ...
  return (
    <div className={cn(
      "rounded-xl border-2 bg-white p-4 sm:p-6 shadow-sm transition-all hover:shadow-md",
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center space-x-2 sm:space-x-3">
          <div className={cn(
            "flex h-8 w-8 sm:h-12 sm:w-12 items-center justify-center rounded-lg",
      ...
      <div className="mt-3 sm:mt-4 space-y-1.5 sm:space-y-2">
        <div className="flex items-center justify-between rounded-lg bg-purple-50 p-2 sm:p-3">
          <div>
            <p className="text-xs text-purple-600">{t('pendingEclv')}</p>
            <p className="mt-1 text-sm sm:text-lg font-semibold text-purple-900">
        ...
        <div className="flex items-center justify-between rounded-lg bg-green-50 p-2 sm:p-3">
          <div>
            <p className="text-xs text-green-600">{t('pendingUsdt')}</p>
            <p className="mt-1 text-sm sm:text-lg font-semibold text-green-900">
      ...
      {isLive && dissolutionProposal?.exists && !dissolutionProposal.executed && (
        <div className="mt-3 sm:mt-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <span className="text-sm font-medium text-orange-800">解散提议进行中</span>
          </div>
          ...
          <button
            ...
            className="w-full rounded-lg bg-orange-600 text-white px-3 py-2 text-xs font-medium hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-1"
          >
      ...
      <div className="mt-3 sm:mt-4 flex space-x-1.5 sm:space-x-2">
        <Link
          href={`/nfts/${nftId}`}
          className="flex-1 rounded-lg border border-gray-300 bg-white px-3 sm:px-4 py-1.5 sm:py-2 text-center text-xs sm:text-sm font-medium text-gray-700 hover:bg-gray-50"
        >
          {tCommon('details')}
        </Link>
        <Link
        ...
        {isLive && !dissolutionProposal?.exists && unlockedPercentage > 0 && (
          <button
            ...
            className="flex-1 rounded-lg border border-red-300 bg-red-50 px-3 sm:px-4 py-1.5 sm:py-2 text-center text-xs sm:text-sm font-medium text-red-700 hover:bg-red-100 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-1"
          >
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto max-w-7xl px-3 sm:px-6 lg:px-8 py-4 sm:py-8">
        ...
        <div className="mb-4 sm:mb-8 flex items-center justify-between gap-4">
          ...
          <Link
            href="/mint"
            className="shrink-0 inline-flex items-center justify-center space-x-1 sm:space-x-2 rounded-lg bg-gradient-to-r from-blue-500 to-purple-600 px-2 sm:px-4 py-2 text-sm font-semibold text-white shadow-sm hover:from-blue-600 hover:to-purple-700 transition-colors"
            title="快速铸造"
          >
        ...
        {!isConnected ? (
          ...
          <div className="rounded-lg border-2 border-dashed border-gray-300 bg-white p-6 sm:p-12 text-center">
            ...
            <Link
              href="/mint"
              className="mt-4 sm:mt-6 inline-flex items-center space-x-2 rounded-lg bg-gradient-to-r from-blue-500 to-purple-600 px-4 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm font-semibold text-white shadow-sm hover:from-blue-600 hover:to-purple-700"
            >
          ...
          <>
            ...
            {web3Data.nfts.length > 1 && (
              <div className="mb-4 sm:mb-6 flex items-center justify-between rounded-lg bg-white p-3 sm:p-4 shadow-sm">
                <div>
                ...
                <button
                  ...
                  className="inline-flex items-center space-x-1 sm:space-x-2 rounded-lg bg-purple-600 px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-medium text-white hover:bg-purple-700 disabled:opacity-50"
                >
                  {isBatchClaiming ? (
                    <>
                      <Loader2 className="h-3 w-3 sm:h-4 sm:w-4 animate-spin" />
