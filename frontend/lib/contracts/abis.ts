...
import EnclaveTokenABI from './enclave-token-abi.json';
import TokenVestingABI from './token-vesting-abi.json';
...
// 使用真实的NFTManager ABI (Diamond Pattern)
// JSON file may be an object with .abi property (from backend generation) or direct array
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
// 使用真实的EnclaveToken ABI
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const ENCLAVE_TOKEN_ABI = normalizeABI(EnclaveTokenABI) as any[];

// 使用真实的TokenVesting ABI
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const TOKEN_VESTING_ABI = normalizeABI(TokenVestingABI) as any[];
