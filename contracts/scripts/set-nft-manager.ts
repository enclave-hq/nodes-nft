...
async function main() {
  console.log("🔧 Setting NFTManager address in NodeNFT contract\n");
  ...
  // Set new NFTManager address
  console.log("\n📝 Setting NFTManager address in NodeNFT...");
  try {
    const tx = await nodeNFT.setNFTManager(NFT_MANAGER_ADDRESS);
    console.log("   Transaction hash:", tx.hash);
    console.log("   Waiting for confirmation...");
    
    const receipt = await tx.wait();
    console.log("✅ NFTManager address set successfully!");
    ...
    console.error("❌ Failed to set NFTManager address:");
